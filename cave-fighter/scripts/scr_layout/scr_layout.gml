...
// Super class that all layout elements use
function __LayoutSuper() constructor
{
	...
	function DefineControl(_source, _varpatharray, _varindex = -1)
	{
		...
		{
			if !is_string(_varpatharray)
			{
				show_debug_message("DefineControl() ERROR: variable path is not string.");
				return self;
			}
	...
	function DrawRectWH(x, y, w, h, color, alpha=1)
	{
		...
		else
		{
			...
			draw_rectangle_color(x, y, x+w, y+h, c_dkgray, c_dkgray, c_dkgray, c_dkgray, 0);
...
function Layout() : __LayoutSuper() constructor
{
	...
	function Draw()
	{
		...
		var _lastalign = [draw_get_halign(), draw_get_valign()];
		...
		// Restore Draw State
		display_set_gui_size(_guidimensions[0], _guidimensions[1]);
		draw_set_halign(_lastalign[0]);
		draw_set_valign(_lastalign[1]);
