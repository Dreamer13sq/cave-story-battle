...
{
	var _chr = string_char_at( _str, string_pos("%", _str) + 1);
	...
	switch(_chr)
	{
		...
		// Float w/ 2 places
		case("f"):
			return string_replace(_str, "%f", string_format(_value, 1, 2));
		...
		// Float w/ 8 places
		case("r"):
			return string_replace(_str, "%r", string_format(_value, 1, 8));
			
		// Float w/ 16 places
		case("R"):
			return string_replace(_str, "%R", string_format(_value, 1, 16));
		...
		case("m"):
			...
			repeat(16)
			{
				out += string_format(_value[4 * (i mod 4) + (i div 4)], 4, 2);
				i++;
				if (i mod 4) == 0 {out += "\n";}
				else {out += ", ";}
			}
			return string_replace(_str, "%m", out);
...
function stringf()
{
	var out = string(argument[0]), i = 1;
		
	// While %'s exist in string...
	while ( string_count("%", out) && i < argument_count )
	{
		out = updatef(out, argument[i]);
		i++;
	}
